...
def get_aggregated_oi_analysis(db: Session = Depends(get_db)):
    ...
    try:
        ...
        for sym, dates_dict in sym_data.items():
            ...
            if prev_price is None or prev_price == 0: prev_price = curr_price

            if curr_price == 0 and curr.get("oi") == 0:
                continue # Skip entirely empty records

            price_chg = ((curr_price - prev_price) / prev_price) * 100 if prev_price > 0 else 0

            prev_oi = prev.get("oi")
            if prev_oi is None or prev_oi == 0: prev_oi = curr.get("oi")

            oi_chg = ((curr["oi"] - prev_oi) / prev_oi) * 100 if prev_oi > 0 else 0
            ...
            if sym in hist_data:
                ...
                for i in range(len(sorted_hist_dates)):
                    ...
                    ch_p = curr_h["price"] if curr_h["price"] is not None else 0.0
                    ph_p = prev_h["price"] if prev_h and prev_h["price"] is not None and prev_h["price"] > 0 else ch_p

                    if ph_p > 0:
                        h_price_chg = ((ch_p - ph_p) / ph_p) * 100

                    ph_oi = prev_h["oi"] if prev_h and prev_h["oi"] > 0 else curr_h["oi"]
                    if ph_oi > 0:
                        h_oi_chg = ((curr_h["oi"] - ph_oi) / ph_oi) * 100
                ...
                for label, idx in timeframes:
                    if idx < len(all_hist_dates):
                        ...
                        if past_dt in hist_data[sym]:
                            ...
                            cdp = c_data["price"] if c_data["price"] is not None else 0.0
                            pdp = p_data["price"] if p_data["price"] is not None else 0.0

                            if pdp > 0:
                                adv_metrics[f"price_chg_{label}d"] = ((cdp - pdp) / pdp) * 100
                            if p_data["oi"] > 0:
