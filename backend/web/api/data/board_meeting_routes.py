...
def override_board_meeting_xml(req: OverrideXMLRequest, db: Session = Depends(get_db)):
    ...
    # Fetch XML content
    if req.xml_url.startswith("http"):
        try:
            headers = {
        ...
        # Offline local path handling
        try:
            with open(req.xml_url, "r", encoding="utf-8") as f:
                xml_text = f.read()
        except Exception as e:
            raise HTTPException(status_code=400, detail=f"Error reading local file: {e}")
