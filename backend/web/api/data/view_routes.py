...
def get_live_price(symbol: str = Query(..., min_length=1), db: Session = Depends(get_db)):
    ...
    try:
        ...
        # 2. Fetch Futures from BasisWatchMetrics permanently
        near_fut = 0.0
        next_fut = 0.0
        far_fut = 0.0

        latest_basis = db.query(BasisWatchMetrics).filter(
            BasisWatchMetrics.symbol == symbol.upper()
        ).order_by(BasisWatchMetrics.trade_date.desc()).first()

        if latest_basis:
            near_fut = latest_basis.near_fut_close or 0.0
            next_fut = latest_basis.next_fut_close or 0.0
            far_fut = latest_basis.far_fut_close or 0.0
