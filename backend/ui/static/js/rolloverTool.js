const RolloverTool = {
                ...
                <div style="display: flex; gap: 15px; margin-bottom: 15px; align-items: center; flex-shrink: 0;">
                    ...
                    <div style="display: flex; align-items: center; gap: 10px;">
                        <div style="position: relative;">
                            <input type="text" id="rollover-symbol" class="form-control history-input" placeholder="Search/Filter Symbol" style="width: 150px; padding: 4px; padding-right: 20px;">
                            <span id="rollover-clear-search" style="position: absolute; right: 5px; top: 50%; transform: translateY(-50%); cursor: pointer; color: #888; display: none;" onclick="document.getElementById('rollover-symbol').value='NIFTY'; RolloverTool.analyzeSingle(); this.style.display='none';">✖</span>
                        </div>
  ...
  syncAndLoadAggregatedData: async function (forceMasterSync = false) {
    ...
    try {
      const syncRes = await fetch(
        `/api/data/analysis/rollover/sync?force=${isForce}`,
        { method: "POST" },
      );
  ...
  loadAggregatedData: async function () {
    ...
    const matrixContainer = document.getElementById(
      "rollover-matrix-container",
    );

    if (matrixContainer) {
      matrixContainer.innerHTML =
        '<p style="text-align:center; color:#888; padding: 20px;">Fetching F&O Rollover history...</p>';
    }

    try {
      let days = "14";
      const daysRadio = document.querySelector(
        'input[name="rollover_days"]:checked',
      );
      if (daysRadio) {
      ...
      const res = await fetch(
        `/api/data/analysis/rollover?days=${days}&expiry_only=${isMoM}`,
      );
      ...
      // Render Matrix using the fetched data
      this.renderMatrix(this.allData, isMoM);
    } catch (e) {
      if (matrixContainer) {
        matrixContainer.innerHTML = `<p style="text-align:center; color:red; padding: 20px;">Error: ${e.message}</p>`;
      }
    }
  },
  ...
  renderMatrix: function (data, isMoM) {
    ...
    if (dates.length === 0) {
      container.innerHTML =
        '<p style="padding: 10px; color:#888;">No history available to build matrix.</p>';
      return;
    }
    ...
    sortedData.forEach((item) => {
      ...
      displayDates.forEach((date) => {
        ...
        if (h) {
          const roll =
            h.rollover_pct !== null ? h.rollover_pct.toFixed(1) + "%" : "-";
          const spread =
            h.rollover_cost !== null ? h.rollover_cost.toFixed(2) : "-";

          let bps = "-";
          const fPrice = h.price !== undefined ? h.price : h.fut_price;
          if (h.rollover_cost !== null && fPrice !== null && fPrice > 0) {
            bps = ((h.rollover_cost / fPrice) * 10000).toFixed(1);
          }
