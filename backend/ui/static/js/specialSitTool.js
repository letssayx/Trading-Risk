...
function handleOFSFutureSelectionChange() {
    ...
    if (fetchedFutures[sel]) {
        document.getElementById('ofs-hedge-entry').value = parseFloat(fetchedFutures[sel]).toFixed(2);
    }
    calculateOFS();
}
...
function exportSpecialSitCSV(type) {
    ...
    if (type === 'buyback') {
        ...
        csv.push(`Future price,${document.getElementById('ofs-hedge-entry').value}`);
    }
