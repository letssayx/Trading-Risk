                    ...
                    if has_dividend_mention or is_agm:
                        ...
                        if (found_amount is None or found_record_date is None) and bm_date_obj_check and bm_date_obj_check <= datetime.now().date():
                            ...
                            if attachment_url.startswith('http'):
                                pdf_result = extract_amount_from_pdf(attachment_url)
                                if isinstance(pdf_result, dict):
                                    if found_amount is None and pdf_result.get('amount'):
                                        found_amount = pdf_result.get('amount')
                                    if found_record_date is None and pdf_result.get('record_date'):
                                        found_record_date = pdf_result.get('record_date')
                                        logger.warning(f"[PDF_TABLE_FAIL] Record date extracted from table for {symbol}: {found_record_date}")
                                            ...
                                            try:
                                                ...
                                                if bm_date_obj.strftime("%d-%b-%Y").lower() in ann_date_str.lower():
                                                    ...
                                                    if ann_pdf and ann_pdf.startswith('http'):
                                                        pdf_result = extract_amount_from_pdf(ann_pdf)
                                                        if isinstance(pdf_result, dict):
                                                            if found_amount is None and pdf_result.get('amount'):
                                                                found_amount = pdf_result.get('amount')
                                                            if found_record_date is None and pdf_result.get('record_date'):
