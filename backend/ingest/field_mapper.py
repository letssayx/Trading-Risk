...
class FieldMapper:
    ...
    def _map_corporate_actions(cls, df: pd.DataFrame, trade_date: Optional[date]) -> List[Dict]:
        records = []
        for _, row in df.iterrows():
            broadcast_date = parse_nse_timestamp(cls._get_val(row, ['BROADCAST DATE']))
            meeting_date = parse_nse_date(cls._get_val(row, ['MEETING DATE']))
            ex_date = parse_nse_date(cls._get_val(row, ['EX-DATE']))

            record = {
                'date': broadcast_date.date() if broadcast_date else (ex_date or trade_date),
    ...
    def _map_agm(cls, df: pd.DataFrame, trade_date: Optional[date]) -> List[Dict]:
        records = []
        for _, row in df.iterrows():
            # Maps AGM events generated by SmartDividendImporter directly to DividendDatabank format
            broadcast_date = parse_nse_timestamp(cls._get_val(row, ['BROADCAST DATE']))
            ex_date = parse_nse_date(cls._get_val(row, ['EX-DATE']))

            record = {
                'date': broadcast_date.date() if broadcast_date else trade_date,
                'symbol': str(cls._get_val(row, ['SYMBOL'])).strip().upper(),
                'company_name': str(cls._get_val(row, ['COMPANY NAME'])).strip(),
                'purpose': str(cls._get_val(row, ['PURPOSE'])).strip(),
                'board_meeting_broadcast_date': broadcast_date,
    ...
    def _map_board_meetings(cls, df: pd.DataFrame, trade_date: Optional[date]) -> List[Dict]:
        # Both board meetings and corporate actions return the same format now and map to the same place,
        # but to keep FieldMapper's API stable, we just call the CA mapper.
        return cls._map_corporate_actions(df, trade_date)
