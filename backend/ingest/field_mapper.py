    ...
    def _map_board_meetings(cls, df: pd.DataFrame, trade_date: Optional[date]) -> List[Dict]:
        ...
        for _, row in df.iterrows():
            ...
            record = {
                ...
                'parsed_dividend_amount': cls._get_val(row, ['EXTRACTED_DIVIDEND_AMOUNT']),
                'dividend_type': cls._get_val(row, ['EXTRACTED_DIVIDEND_TYPE']),
                'record_date': cls._get_val(row, ['EXTRACTED_RECORD_DATE'])
            }
            if record['symbol'] and record['date']:
                records.append(record)
        return records
