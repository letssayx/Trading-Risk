...
if not thead_search:
    print("Could not find mr-data-head")
    exit()

old_thead = thead_search.group(0)
...
# Same for javascript headers!
js_search_block = r"thead\.innerHTML = `[\s\S]*?`;"
import re

def js_repl(m):
    return f"""thead.innerHTML = `{new_thead.replace('<thead id="mr-data-head">', '').replace('</thead>', '')}`;"""

# But wait, there are TWO places where `thead.innerHTML` is set in workbench.html!
# We need to make sure we replace the correct one inside the JS block `if (!snapshotMode)`
# Let's just do it directly.

with open('backend/ui/templates/workbench.html', 'w') as f:
    f.write(html)

print("Patched thead HTML")
