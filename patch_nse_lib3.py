...
# RECLTD "31-jul-2026" description says "Record date for the purpose of Dividend is 31-Jul-2026."
# And earlier in `nse_lib.py` it has: `if 'interim' in text_lower or 'intdiv' in text_lower: found_type = 'Interim'`
# But since this description doesn't explicitly mention "interim", it defaulted to 'Final'.
# Let's fix the type extraction logic by also referencing the Board Meeting's base type since we know one of them was for an interim dividend if the board meeting was for financial results.
# Wait, actually the base_type for the item itself was 'Final' because 'fin div' wasn't there, and it fell back to 'Final' in the `base_type = ...` line.
# Let's let the pipeline logic fix the base_type later via deduplication if needed, OR we can leave it as 'Final' and 'Final' and deduplication will just see a new record date. But wait, if both are 'Final', then deduplication in tasks.py will drop one of them if they are in the same window!
# Let's check RECLTD board meeting purpose. It says: "REC LIMITED has informed the Exchange about Board Meeting to be held on 24-Jul-2026 to consider and approve the Quarterly Unaudited Financial results of the Company for the period ended June 2026 and Dividend."
# Quarterly Unaudited Financial Results -> This usually means Interim.
# We will add logic: if 'quarterly' in purpose/desc, base_type should be 'Interim'.
