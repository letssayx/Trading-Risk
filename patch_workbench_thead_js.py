...
# Find the block inside `if (!snapshotMode)`
pattern = r'(if \(!snapshotMode\) {\s*thead\.innerHTML = `).*?(`;\s*} else {)'
match = re.search(pattern, html, re.DOTALL)
if match:
    html = html.replace(match.group(0), f"{match.group(1)}{new_thead}{match.group(2)}")
    with open('backend/ui/templates/workbench.html', 'w') as f:
        f.write(html)
    print("Patched JS inside HTML successfully.")
else:
