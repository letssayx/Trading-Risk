...
# 1. Update loadAggregatedData to call matrix and update UI:
# Find:
...
"""
js = re.sub(
    r'if \(json\.data && Array\.isArray\(json\.data\)\) \{[\s\S]*?this\.updateDynamicChart\(\);\s*\}',
    load_agg_str.strip(),
    js
)

# 2. Add Matrix functions to the object, before the end.
matrix_funcs = """
...
# 3. Add Matrix HTML container and fix BPS to analyzeSingle table
# Let's add it right under #rollover-results
html_update = """
                <div id="rollover-results" style="flex: 1; overflow: auto; display: flex; flex-direction: column; gap: 20px; padding-bottom: 20px;">
                    ...
                    <!-- Charts Area -->
"""
js = js.replace('<div id="rollover-results" style="flex: 1; overflow: auto; display: flex; flex-direction: column; gap: 20px; padding-bottom: 20px;">\n                    <!-- Charts Area -->', html_update)
...
# 4. Modify analyzeSingle
# We need to add BPS calculations!
...
# Initialize NIFTY
js = js.replace("        if (container && !container.innerHTML.includes('rollover-symbol')) {\n            this.render(container);\n            this.loadAggregatedData();\n        }",
