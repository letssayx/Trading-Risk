...
# Mock imports for now, avoiding complex dependency on analysis/risk packages in this snippet if not strictly needed,
# but plan says to use evaluate_scenario. We will mock the risk function for the engine logic to keep it pure.
...
class BacktestEngine:
    ...
    def run_simulation(self, strategy_config: Dict[str, Any], historical_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        # Mock formula evaluation logic
        # In real system: analysis.formula_engine.compute_custom_formula
        ...
        for index, row in historical_data.iterrows():
            ...
            # Assuming 'signal' column exists or logic is simple
            ...
            if signal_triggered:
                ...
                # PnL impact of a 10% shock
                potential_risk = -0.10 * self.current_capital * 0.1 # 10% of 10% position

                # Check Risk Limits
                if abs(potential_risk) <= strategy_config.get('max_risk_per_trade', 50000):
                    # Simulate Trade Outcome (Next Day Return)
                    pnl = row.get('next_day_return', 0.0) * self.current_capital * 0.1 # 10% allocation
            ...
            # Record Daily State (Simplification)
    ...
    def calculate_metrics(self) -> Dict[str, Any]:
        ...
        # Max Drawdown (Capital Peak to Valley)
